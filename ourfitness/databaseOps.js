...
const getMostRecentDB = "SELECT MAX(rowIdNum), activity, date, amount FROM ActivityTable and userId = ?";
...
const insertUserDB = "insert into Profile (id, name) values (?,?)"
...
async function testDB () {
  ...
  // User id for sample/testing data is my personal Google id
  for(const entry of dbData) {
    for(let i = 0 ; i < entry.data.length; i++) {
      await db.run(insertDB,[entry.type, entry.start + i * MS_IN_DAY, entry.data[i], 108612794564813624201]);
    }
  }
  

  await db.run(insertDB,["yoga", planDate2, -1, 108612794564813624201]);
  await db.run(insertDB,["yoga", planDate3, -1, 108612794564813624201]);
  await db.run(insertDB,["run", planDate2, -1, 108612794564813624201]);
...
async function post_profile(id, name) {
  try {
    let results = await db.all("select * from Profile where id = ?", [id]);
    if (results.length == 0){
      await db.run(insertUserDB, [id, name]);
      console.log("New user created");
    }
    console.log("Profile results here", results);
  } catch (error) {
    console.log("error", error)
  }
}
